...
from src.trans.modeling_flash_attention_utils import _flash_attention_forward
...
class MyAttentionAbsPE(nn.Module):
    """Multi-headed attention with Absolute Positional Encoding"""
...
class MyFlashAttention2AbsPE(MyAttentionAbsPE):
    """FlashAttention2 version with Absolute Positional Encoding"""
    ...
    def forward(
        ...
        # QKV
        query_states = self.q_proj(hidden_states).view(bsz, seq_len, self.num_heads, self.head_dim).transpose(1, 2)
        key_states = self.k_proj(hidden_states).view(bsz, seq_len, self.num_key_value_heads, self.head_dim).transpose(1,
                                                                                                                      2)
        value_states = self.v_proj(hidden_states).view(bsz, seq_len, self.num_key_value_heads, self.head_dim).transpose(
            1, 2)
        ...
        attn_output = _flash_attention_forward(
            ...
            attention_mask, seq_len, dropout=dropout_rate,
            use_top_left_mask=self._flash_attn_uses_top_left_mask,
            is_causal=self.is_causal,
            **kwargs
        )
